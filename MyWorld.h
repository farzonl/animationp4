...
#define IXColor(i, j, c) ((i)*(3)*(getNumCells()+2)+(j)*(3)+(c))
//#define IXColor(i, j, c) ((getNumCells()+2)*((getNumCells()+2)*c+(j))+(i))
...
class MyWorld {
 public:
    ...
    void setDensityColor(int _i, int _j, int c, double _source) { mDensity[IXColor(_i, _j, c)] += mTimeStep * _source; }
